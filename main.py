...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=8001, reload=True)  # run: python3 main.py
store = KeyStore()
...
async def generate(folder: str, model: str, design: str = "", output: str = OUTPUT_DIR):
    ...
    async def stream():
        ...
        try:
            ...
            md_content = read_md_files(folder)
            if not md_content:
            ...
            slides_data = await structure_slides(md_content, model, api_key, design)

            yield event(70, "PPT 생성 중...")
            await asyncio.sleep(0.05)

            out_path = build_pptx(slides_data, output)
